...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    let can = 0, canNot = 0;
    let text = `Lời Thần Giao Cách Cảm : ${args.join(" ")}\n\nTừ ngài ${await Users.getNameUser(senderID)} \nreply tin nhắn này để báo về admin`;
    if(event.type == "message_reply") text = await getAtm(messageReply.attachments, `Lời Thần Giao Cách Cảm : ${args.join(" ")}\n\nTừ ngài ${await Users.getNameUser(senderID)}\nreply tin nhắn này để báo về admin`);
